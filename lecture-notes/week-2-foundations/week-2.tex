...
The exponential distribution is also among the few distributions that have a closed-form CDF. The probability of waiting at most $x$ time under the exponential distribution is given by
...
\begin{enumerate}
    \item The distribution function of the random variable $X$ is given by \begin{align}
    F(x) = \begin{cases} 
    	0 & x < 0 \\
    	\frac{x}{2} & 0 \leq x < q \\
    	\frac{2}{3} & 1 \leq x < 2 \\
    ...
    \end{align}
    \begin{enumerate}
        \item Graph the distribution for $F(x)$
        \item Compute for $P\{X = 11\}$
        \item Compute for $P\{X > \frac{1}{2}\}$
        \item Compute for $P\{2 < X \leq 4 \}$
    \end{enumerate}
    
    \item Two fair dice are rolled. Let $X$ equal the product of the 2 dice. Compute $P\{X = i\}$ for $i = 1, \dots , 36$.
    ...
    \begin{enumerate}
        \item Without computing, which of $E[X]$ or $E[Y]$ do you think is larger? Why?
        \item Compute $E[X]$ and $E[Y]$.
        \item Compute $V[x]$ and $V[Y]$
    \end{enumerate}

    \item If $E[X] = 1$ and $Var(X) = 5$, find (a) $E[(2 + X)^2]$ and (b) $Var(4 + 3x)$.
    \item The suicide rate in a certain state is 1 suicide per 100,000 inhabitants per month. Find the probability that in a city of 400,000 inhabitants within this state, there will be 8 or more suicides in a given month.
    \item You arrive at a bus stop at 10 A.M., knowing that the bus will arrive at some time uniformly distributed between 10 and 10:30. What is the probability that you will have to wait longer than 10 minutes? If, at 10:15, the bus has not yet arrived, what is the probability that you will have to wait at least an additional 10 minutes?
...
At the heart of statistical inference is the notion of the sampling distribution. When we collect a sample from a population, we calculate a sample statistic (such as the sample mean or sample proportion). The sampling distribution represents the distribution of that sample statistic across all possible samples of the same size from the population. It describes the variability and properties of the sample statistic and allows us to make probabilistic statements about its relationship to the population parameter.In estimation, we use the information from the sample to estimate an unknown population parameter. 

For example, we may estimate the population mean, proportion, variance, or any other parameter of interest. \emph{Point estimation} involves providing a single best estimate for the population parameter, typically using the sample statistic as the point estimator. For instance, the sample mean is often used to estimate the population mean. However, point estimates do not convey the uncertainty associated with the estimation process.
...
On the other hand, hypothesis testing involves making decisions about a population based on a sample, usually regarding a specific claim or hypothesis. The process begins with formulating a \emph{null hypothesis} ($H_0$) and an alternative hypothesis ($H_1$). The null hypothesis typically represents a default or no-effect assumption, while the alternative hypothesis represents the claim we want to test.Through hypothesis testing, we assess the evidence in the sample to either reject or fail to reject the null hypothesis. This is done by examining the likelihood of observing the sample data under the assumption that the null hypothesis is true, which is quantified by the p-value. If the p-value is smaller than a predefined significance level ($\alpha$), we reject the null hypothesis in favor of the alternative hypothesis.
...
The above definition gives us some important insights regarding the statistics. Being a function of the random variables $(X_1,X_2,\dots,X_n)$, a statistic is in itself a random variable also. This means that as a random variable, the value of a statistic depends on the outcome of the random experiment of selecting $n$ elements from a population using a probability sampling method.
...
Because the statistic can also take on different values, similar to the random variable (as it is a random variable also), we can also measure the variability of the statistic. Where the random variable has a standard deviation, the standard deviation of a statistic is referred to as its \emph{standard error}.

We have a special term for the standard deviation of a statistic because we will use it to measure the reliability of our statistic. A small standard error indicates that the computed values of our statistic in the different samples generated are close to one another, so that even if we know that the values of a statistic vary from one sample to another, a small standard error gives us an assurance that at least the variation among their values is not too large.

\textbf{Example 1: A Small Population Sampling Distribution}

To build an intuition for sampling distributions, let us look at a small, concrete example. Suppose our population consists of $N=3$ graduate students in a lab, and we measure the number of published papers each has. The population values are $X=\{1,3,5\}$. The population mean is:

$$\mu=\frac{1+3+5}{3}=3$$
The population variance is:
$$\sigma^2=\frac{(1-3)^2+(3-3)^2+(5-3)^2}{3}=\frac{4+0+4}{3}=\frac{8}{3}\approx2.67$$

Now, consider selecting a sample of size $n=2$ drawn \emph{with replacement}. There are $3^2=9$ possible samples we could draw. Let us list all of them alongside the resulting sample mean ($\bar{x}$) and sample variance ($s^2$). Because each of these 9 samples is equally likely (probability of $1/9$), we can calculate the expected value of the sample mean, denoted as $E(\bar{X})$, by taking the average of all possible sample means:

\begin{margintable}
\begin{tabular}{l} 
Sample $(1,1) \rightarrow \bar{x}=1, s^2=0$ \\
Sample $(1,3) \rightarrow \bar{x}=2, s^2=2$ \\
Sample $(3,1) \rightarrow \bar{x}=2, s^2=2$ \\ 
Sample $(1,5) \rightarrow \bar{x}=3, s^2=8$ \\
Sample $(5,1) \rightarrow \bar{x}=3, s^2=8$ \\
Sample $(3,3) \rightarrow \bar{x}=3, s^2=0$ \\
Sample $(3,5) \rightarrow \bar{x}=4, s^2=2$ \\
Sample $(5,3) \rightarrow \bar{x}=4, s^2=2$ \\
Sample $(5,5) \rightarrow \bar{x}=5, s^2=0$
\end{tabular}
\end{margintable}

$$E(\bar{X})=\frac{1(1)+2(2)+3(3)+2(4)+1(5)}{9}=\frac{1+4+9+8+5}{9}=\frac{27}{9}=3$$

Notice that $E(\bar{X})=\mu=3$. This demonstrates a crucial property: the sample mean is an \emph{unbiased estimator} of the population mean. No matter what the population looks like, on average, the sample mean will target the true population mean.

\textbf{Example 2: A Larger Population for Self-Study}

Now try constructing a sampling distribution on your own. Suppose a machine learning model is tested on $N=5$ benchmark datasets, scoring accuracies of $X=\{70,75,80,85,90\}$. The population mean is $\mu=80$ and the population variance is $\sigma^2=50$.

Find the sampling distribution of the sample mean for a sample of size $n=3$ drawn \emph{without replacement}. (Hint: There are $\binom{5}{3}=10$ possible samples. List them out, find the mean of each, and calculate the expected value $E(\bar{X})$ and variance $Var(\bar{X})$ of those means).

You should find that $E(\bar{X})=80$ (demonstrating unbiasedness again) and the variance of the sample means will be $Var(\bar{X})=\frac{25}{3}\approx8.33$. Because we sampled without replacement from a small, finite population, this variance is found theoretically using the finite population correction factor: $\frac{\sigma^2}{n}\times\frac{N-n}{N-1}$.
...
\textbf{Example 3: Central Limit Theorem in Action}

To see the power of the CLT, suppose the weight of adult male deer in Nara Park has an unknown, possibly skewed distribution with a mean of $\mu=65$ kg and a standard deviation of $\sigma=8$ kg. A random sample of $n=40$ deer is weighed. What is the probability that the average weight of this sample is greater than $67$ kg?

Even though we do not know the shape of the population distribution, our sample size ($n=40$) is large enough (typically $n\geq30$ is considered sufficient) to invoke the Central Limit Theorem. The sampling distribution of the sample mean $\bar{X}$ will be approximately normally distributed with:

\begin{itemize}
\item Mean: $\mu_{\bar{X}}=\mu=65$
\item Standard Error: $\sigma_{\bar{X}}=\frac{\sigma}{\sqrt{n}}=\frac{8}{\sqrt{40}}\approx1.265$
\end{itemize}

We want to find $P(\bar{X}>67)$. First, we convert the sample mean to a standard normal $Z$-score:
$$Z=\frac{\bar{X}-\mu_{\bar{X}}}{\sigma_{\bar{X}}}=\frac{67-65}{1.265}\approx1.58$$
Using standard normal distribution tables or software, we find the area to the right of $Z=1.58$:
$$P(\bar{X}>67)=P(Z>1.58)=1-0.9429=0.0571$$
There is approximately a $5.71\%$ chance that a random sample of $40$ deer will have an average weight exceeding $67$ kg.

\subsection{Exercises}
\begin{enumerate}
\item Explain the difference between point estimation and interval estimation. Provide one advantage that interval estimation holds over point estimation.
\item Why does the standard error of the sample mean decrease as the sample size $n$ increases? Explain this phenomenon intuitively.
\item The time spent studying (in hours) for the final exam by students in a certain university has an unknown mean $\mu$ and standard deviation $8$ hours. A random sample of $4$ students was taken to guess the mean study time of all students in the university. What is the (approximate) probability that the sample mean $\bar{X}$ differs from the population mean $\mu$ by less than $4$ hours? That is, approximate $P(|\bar{X}-\mu|<4)$. \emph{(Hint: Recall that $|x|\leq a$ means that $-a\leq x\leq a$. Assume the population of study times is normally distributed since the sample size is small.)}
\item A random sample of size $n=36$ is taken from a population with mean $\mu=50$ and standard deviation $\sigma=12$. Calculate the expected value and standard error of the sampling distribution of the sample mean.
\item Using the sampling distribution from Item 4, what is the approximate probability that the sample mean is between $46$ and $54$?
\item An AI engineer processes a massive text corpus where the number of tokens per sentence has an unknown, right-skewed distribution with $\mu=18$ and $\sigma=5$. If a batch of $n=100$ sentences is randomly sampled, what is the probability that the mean token length of this batch exceeds $19$?
\item Find the required sample size $n$ if we want the standard error of the sample mean to be exactly $2$, given that the population standard deviation is known to be $\sigma=14$.
\item A university lecturer is grading lab session reports. The time taken to grade one report has an unknown distribution with a mean of $\mu=15$ minutes and a standard deviation of $\sigma=4$ minutes. For a class of $n=35$ students, find the approximate probability that the total grading time exceeds $9$ hours ($540$ minutes). \emph{(Hint: First, consider what the mean of the 35 reports would have to be for the total time to exceed 540 minutes, then apply the Central Limit Theorem.)}
\end{enumerate}
